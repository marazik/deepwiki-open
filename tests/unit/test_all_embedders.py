...
class TestEmbedderConfiguration:
    ...
    def test_config_loading(self):
        ...
        assert (
            "OpenAIClient" in CLIENT_CLASSES
        ), "OpenAIClient missing from CLIENT_CLASSES"
        assert (
            "GoogleEmbedderClient" in CLIENT_CLASSES
    ...
    def test_get_embedder_config(self, embedder_type=None):
        ...
        if embedder_type:
            ...
            with patch("api.config.EMBEDDER_TYPE", embedder_type):
                ...
                assert (
                    "model_client" in config or "client_class" in config
...
class TestEmbedderClients:
    ...
    def test_openai_embedder_via_adalflow(self):
        ...
        if not os.getenv("OPENAI_API_KEY"):
            logger.warning(
                "Skipping OpenAI embedder test - OPENAI_API_KEY not available"
            )
            return
...
class TestEnvironmentVariableHandling:
    ...
    def _test_single_embedder_type(self, embedder_type):
        ...
        try:
            ...
            assert (
                get_embedder_type() == embedder_type
